	...
	public function index(): void {
		...
		if ($product_info) {
			...
			$data['header'] = $this->load->controller('common/header');
				...
				$is_hide_enabled = $this->db->query("select product_id from ".DB_PREFIX."product where add_cart = 0 and product_id = ".$this->request->get['product_id']);
				if($is_hide_enabled->num_rows){
					$data['cart_products'] = 1;
				}else{
					$data['cart_products'] = 0;
				}
				
				// Out of Stock Products
				$is_products_out_stock = $this->db->query("select product_id from  ".DB_PREFIX."product where quantity = 0 and product_id = ".$this->request->get['product_id']);
				if(!empty($is_products_out_stock->num_rows)){
					$data['out_stock_products'] = 1;
				}else{
					$data['out_stock_products'] = 0;
				}
				
				$data['hide_cart_out_stock'] = $this->config->get('module_hide_cart_out_of_stock_status');

				// Contact Us
				$data['contact'] = $this->url->link('information/contact', 'product_id=' . $this->request->get['product_id'] . '&product_name='. $product_info['name']);
